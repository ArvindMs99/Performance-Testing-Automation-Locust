    ...
    def post_user(self):
        ...
        "job": "leader"
            ...
            if self.expected_post_response_text in response.text and self.expected_post_statuscode == response.status_code:
                logging.info("POST REQUEST FILE3 PASSED")
                response.success()
            else:
                logging.error("POST REQUEST FILE3 FAILED")
                response.failure()
    ...
    def patch_user(self):
       ...
       "job": "zion resident"
            ...
            if self.expected_patch_response_text in response.text and self.expected_patch_statuscode == response.status_code:
                logging.info("PATCH REQUEST FILE3 PASSED")
                response.success()
            else:
                logging.error("PATCH REQUEST FILE3 FAILED")
                response.failure()
    ...
    def delete_user(self):
        ...
        with self.client.delete("/api/users/2", catch_response=True) as response:
            ...
            if self.expected_delete_statuscode == response.status_code:
                logging.info("DELETE REQUEST FILE3 PASSED")
                response.success()
            else:
                logging.error("DELETE REQUEST FILE3 FAILED")
                response.failure()


class TestsRequests3(HttpUser):
    tasks = [TasksRequests]
